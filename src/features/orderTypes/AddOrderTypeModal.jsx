...
import { useCreateOrderTypeMutation } from "src/api/orderType";

const iconProps = { display: "block", size: 80, strokeWidth: 1.25 };

const AddOrderTypeModal = ({ isOpen = false, onClose = () => {} }) => {
  const createOrderTypeMutation = useCreateOrderTypeMutation();

  const form = useForm({ initialValues: { title: "", file: null } });
  ...
  const handleSubmit = (values) => {
    ...
    createOrderTypeMutation.mutate(formData, {
      onSuccess: () => {
        onClose();
        form.reset();
      },
  ...
  return (
    <Modal centered title={"create order type"} tt={"capitalize"} opened={isOpen} onClose={onClose}>
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack>
          ...
          <Button type="submit" mt="md" loading={createOrderTypeMutation.isPending}>
            Create order type
          </Button>
        </Stack>
      </form>
